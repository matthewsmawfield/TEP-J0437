...
def load_citation_metadata():
    ...
    if not citation_file.exists():
        ...
        return {'version': 'v0.3', 'codename': 'Sintra', 'title': 'TEP-J0437'}
    ...
    try:
        ...
        # Parse version string like 'v0.3 (Sintra)'
        pattern = r'^(v?[\d.]+)(?:\s*\(([^)]+)\))?$'
...
def copy_pdf_to_docs(source_pdf: Path, docs_dir: Path):
    ...
    # Primary PDF name (using project naming convention: paper number + version + location)
    target_name = f"16-TEP-J0437-{version_str}.pdf"
...
async def generate_pdf(quality: str = 'high', wait_time: float = 5.0, skip_build: bool = False):
    ...
    async with HTMLToPDFConverter() as converter:
        ...
        # Process with metadata (compresses and embeds metadata)
        process_pdf_with_metadata(final_pdf)
        
        # Copy compressed PDF to root (overwrite uncompressed copy)
        root_pdf = base_dir / final_pdf.name
        import shutil
        shutil.copy2(final_pdf, root_pdf)
        print(f"📄 Compressed root PDF: {root_pdf}")
